...
async def test_vlm_base_url_sends_the_caption_request_to_that_server(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setenv("DATA_DIR", str(tmp_path))
    ...
    async with (
        ...
        with respx.mock() as mock:
            caption = mock.post("http://vlm.test:9000/v1/chat/completions").mock(
                return_value=chat_reply("A red photo.")
            )
            response = await frame.put(
